...
type OutputRow = { product_id: string; quantity_per_batch: string };

const initialState: FormState = { error: null };
const emptyIngredientRow: IngredientRow = { raw_material_id: "", quantity_per_unit: "" };
const emptyOutputRow: OutputRow = { product_id: "", quantity_per_batch: "" };
...
export function CreateRecipeForm({
  ...
  const outputsJson = JSON.stringify(
    outputRows
      .filter((r) => r.product_id && Number(r.quantity_per_batch) > 0)
      .map((r) => ({
    ...
    <form action={formAction} className="space-y-3">
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-neutral-700">
          Barang jadi dihasilkan (per 1 batch)
        </p>
          ...
          {outputRows.map((row, i) => (
            <div key={i} className="flex gap-2">
              ...
              <input
                ...
                onChange={(e) => updateOutputRow(i, { quantity_per_batch: e.target.value })}
